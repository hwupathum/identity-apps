...
import { Checkbox, Divider, DropdownProps, Header, SemanticICONS } from "semantic-ui-react";
import "./user-attribute-list.scss";

/**
/**
 * Interface for the selected user attribute.
 */
export interface SelectedUserAttributeInterface {
    claimURI: string;
    mandatory: boolean;
}

/**
 * Prop types for the user attribute list component.
 */
export interface UserAttributeListPropsInterface extends IdentifiableComponentInterface {
    /**
     * Previously saved attribute list.
     */
    initialValues: SelectedUserAttributeInterface[];
    /**
     * Triggers on attribute change to pass the selected attributes to the parent component.
     */
    onAttributesChange: (hasChanged: boolean, selectedUserAttributes: SelectedUserAttributeInterface[]) => void;
    /**
...
/**
 * Component to handle user attribute selection.
 */
const UserAttributeList: FunctionComponent<UserAttributeListPropsInterface> = ({
    ...
    const [selectedAttributeList, setSelectedAttributeList] = useState<(Claim & { mandatory?: boolean })[]>([]);
    ...
    useEffect(() => {
        ...
        if (!allAttributesList?.length || isInitialized) {
            return;
        }

        const initialURIs: string[] = initialValues?.map(attr => attr.claimURI) || [];

        if (initialURIs.length > 0) {
            // Map initial values to claims and preserve mandatory status
            const mergedList = allAttributesList
                .filter((claim: Claim) => initialURIs.includes(claim.claimURI))
                .map((claim: Claim) => {
                    const initialValue = initialValues.find(attr => attr.claimURI === claim.claimURI);

                    return {
                        ...claim,
                        mandatory: initialValue?.mandatory || false
                    };
                });

            setSelectedAttributeList(mergedList);
        }

        setIsInitialized(true);
    }, [initialValues, allAttributesList, isInitialized]);
    ...
    useEffect(() => {
        ...
        const formattedList: SelectedUserAttributeInterface[] = selectedAttributeList.map(claim => ({
            claimURI: claim.claimURI,
            mandatory: !!claim.mandatory
        }));

        isInitialAttributesChanged() ?
            onAttributesChange(true, formattedList) :
            onAttributesChange(false, formattedList);
    ...
    const isInitialAttributesChanged = (): boolean => {
        ...
        const sortedFinalValues = selectedAttributeList.map(claim => ({
            claimURI: claim.claimURI,
            mandatory: !!claim.mandatory
        })).sort((a, b) => a.claimURI.localeCompare(b.claimURI));

        const sortedInitialValues = [...(initialValues || [])].sort((a, b) => a.claimURI.localeCompare(b.claimURI));

        if (sortedInitialValues.length !== sortedFinalValues.length) {
            return true;
        }

        return !sortedFinalValues.every((attribute, index) =>
            attribute.claimURI === sortedInitialValues[index].claimURI &&
            attribute.mandatory === sortedInitialValues[index].mandatory
        );
    };
     ...
     */
    const isMaxAttributesConfigured = (attributesList: (Claim & { mandatory?: boolean })[]): boolean => {

        if (attributesList?.length === 10) {
    ...
    const handleAttributeSelect = (data: DropdownProps) => {
        ...
        setSelectedAttributeList([...selectedAttributeList, { ...(data as Claim), mandatory: false }]);
    };
    ...
    const handleAttributeDelete = (item: Claim) => {
        ...
        const attributeList = selectedAttributeList?.filter((claim) =>
    ...
    /**
     * Handles the mandatory checkbox change.
     *
     * @param claim - Claim.
     * @param checked - Checked status.
     */
    const handleMandatoryChange = (claim: Claim, checked: boolean) => {
        const attributeList = selectedAttributeList.map((item) => {
            if (item.claimURI === claim.claimURI) {
                return { ...item, mandatory: checked };
            }
        ...
        return [
            {
                ...
                onClick: (e: SyntheticEvent, claim: Claim & { mandatory?: boolean }): void =>
        ...
        return [
            {
                ...
                id: "displayName",
                key: "displayName",
                render: (claim: Claim & { mandatory?: boolean }) => {
            ...
            },
            {
                ...
                key: "mandatory",
                render: (claim: Claim & { mandatory?: boolean }) => (
                    <div className="mandatory-checkbox-container">
                        <Checkbox
                            label="Mandatory"
                            checked={claim.mandatory}
                            onChange={(event, data) => {
                                handleMandatoryChange(claim, data.checked);
                            }}
                            disabled={isReadOnly}
                            data-componentid={`${componentId}-mandatory-checkbox`}
                        />
                    </div>
                ),
                textAlign: "right",
    ...
    return (
        <>
            <Hint>{t("actions:fields.userAttributes.hint")}</Hint> {/* TODO: change */}
            <Autocomplete
                disablePortal
                    ...
                    selectedAttributeList?.length > 0 && (
                        <div className="user-attribute-list">
                            ...
                            <div className="selected-attributes-list">
                                <DataTable<(Claim & { mandatory?: boolean })[]>
